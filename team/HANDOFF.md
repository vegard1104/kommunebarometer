...
| Backend | Prosjektleder | **Pakke 17 (C3) — Udir Statistikkbank skeleton (draft).** kbUdir-helpers + lenke til Udir Statistikkportalen. Vercel-rewrite til api.udir-statistikkbanken.no må legges til. | `data/udir-helper.js`, branch `feature/udir-skolefaglig` | 2026-04-25 |
| Backend | Prosjektleder | **Pakke 10 (B2) — Dynamisk kommuneinndeling (skeleton).** `Kommunestruktur`-klasse henter Klass-API + changes.json, bygger MERGERS/MERGERS_REVERSE-kompatibelt format. 30-dagers cache. Klar for full integrasjon. | `data/kommunestruktur.js`, `index.html`, branch `feature/dynamisk-kommuneinndeling` | 2026-04-25 |
